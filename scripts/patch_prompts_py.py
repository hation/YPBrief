"""Replace the DEFAULT_PROMPTS block in src/ypbrief/prompts.py with the
Simplified-Chinese, force-Chinese-output prompt defaults.

Only touches lines between the DEFAULT_PROMPTS opening line and its closing brace.
"""
...
TARGET = Path("src/ypbrief/prompts.py")

NEW_BLOCK = '''DEFAULT_PROMPTS: dict[str, dict[str, Any]] = {
    "video_summary": {
        "prompt_id": 1,
...
for i, line in enumerate(lines):
    if line.startswith("DEFAULT_PROMPTS: dict[str, dict[str, Any]] = {"):
        start = i
    if start is not None and line.strip() == "}":
        # First closing brace at column 0 after the block started
        end = i
        break

assert start is not None and end is not None, f"block not found start={start} end={end}"

new_lines = lines[:start] + NEW_BLOCK.splitlines() + lines[end + 1:]
TARGET.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
print(f"patched src/ypbrief/prompts.py: replaced lines {start + 1}..{end + 1}")
